...
def process_tcp_run(concurrent_num,ip_str):
    ...
    for ip in parse_ip(ip_str):
        #如果 ping 不通，直接跳过
        if check_ping(str(ip)) == False:
            continue
        for i in range(1024):
            result = p.apply_async(check_tcp,args=(str(ip),str(i+1),))
            #print(f'{str(ip)} --{str(i+1)}-- {result.get()}')
            if result.get() == True:
                print(f'{str(ip)} --{str(i+1)}-- {result.get()}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n',type=int,default=1,metavar=1,help='扫描请求的并发数，默认值为 1')
    parser.add_argument('-f',choices=['ping','tcp'],default='ping',metavar='ping',help='扫描协议：ping 或者 tcp，默认值为ping')
